...
Citizen.CreateThread(function()
    ...
    local meetup_interior_id = GetInteriorAtCoords(vector3(-2000.0, 1113.211, 27.36243))
    ...
    if IsValidInterior(tuna_interior_id) then
        for k, v in pairs(EntitySetsTuner) do
            if v then
                ActivateInteriorEntitySet(tuna_interior_id --[[ integer ]] , k --[[ string ]] )
            else
                DeactivateInteriorEntitySet(tuna_interior_id --[[ integer ]] , k --[[ string ]] )
            end
        end
    ...
    if IsValidInterior(meetup_interior_id) then
        for k, v in pairs(entitySetsMeet) do
            if v then
                ActivateInteriorEntitySet(meetup_interior_id --[[ integer ]] , k --[[ string ]] )
            else
                DeactivateInteriorEntitySet(meetup_interior_id --[[ integer ]] , k --[[ string ]] )
            end
        end
    ...
    if IsValidInterior(methlab_interior_id) then
        for k, v in pairs(EntitySetMeth) do
            if v then
                ActivateInteriorEntitySet(methlab_interior_id --[[ integer ]] , k --[[ string ]] )
            else
                DeactivateInteriorEntitySet(methlab_interior_id --[[ integer ]] , k --[[ string ]] )
            end
        end
